...
def _pluck_msmt(list_file, subdir, pattern=re.compile(r'([-\d]+)_([-\d]+)_([-\d]+)')):
    ...
    for line in lines:
        line = line.strip()
        fname = line.split(' ')[0]
        pid, num, cam = map(int, pattern.search(osp.basename(fname)).groups())
        if pid not in pids:
            pids.append(pid)
        
        img_name = str(pid).zfill(4) + '_c' + str(cam) + '_' + str(num).zfill(4) + '.jpg'

        cam -= 1  # index starts from 0

        ret.append((osp.join(subdir, img_name), pid, cam))
    return ret, pids
