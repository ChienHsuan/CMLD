...
class CameraSampler(Sampler):
    def __init__(self, data_source, num_instances=4):
        ...
        self.pid_index = defaultdict(list)
        self.num_instances = num_instances
        self.unique_cams = set()

        for index, (_, pid, cam) in enumerate(data_source):
            if (pid < 0): continue
            self.pid_cam[pid].append(cam)
            self.unique_cams.add(cam)
            self.pid_index[pid].append(index)

        self.num_classes = len(list(self.pid_index.keys()))
        self.num_cams = len(self.unique_cams)

        self.global_label_table = defaultdict(list)
        global_idx = 0
        for pid_idx in range(self.num_classes):
            for cam_idx in range(self.num_cams):
                self.global_label_table[global_idx].append(pid_idx)
                self.global_label_table[global_idx].append(cam_idx)
                global_idx += 1

    def __len__(self):
        return self.num_classes * self.num_cams * self.num_instances

    def __iter__(self):
        indices = torch.randperm(len(self.global_label_table.keys())).tolist()
        ret = []

        for kid in indices:
            id_pair = self.global_label_table[kid]
            pid = id_pair[0]
            cam_idx = id_pair[1]
            ...
            cam_mask = cams==cam_idx
            cam_num_instances = cam_mask.sum()
            if cam_num_instances == 0:
                select_indexes = np.random.choice(index, size=4, replace=True)
            elif cam_num_instances < self.num_instances:
                select_indexes = np.random.choice(index[cam_mask], size=4, replace=True)
            else:
                select_indexes = np.random.choice(index[cam_mask], size=4, replace=False)
