...
def k_reciprocal_neigh(initial_rank, i, k1):
    forward_k_neigh_index = initial_rank[i,:k1+1]
    backward_k_neigh_index = initial_rank[forward_k_neigh_index,:k1+1]
    fi = torch.nonzero(backward_k_neigh_index==i, as_tuple=False)[:,0]
...
def compute_jaccard_dist(target_features, k1=20, k2=6, print_flag=True, 
    ...
    invIndex = []
    for i in range(gallery_num):
        invIndex.append(torch.nonzero(V[:,i], as_tuple=False)[:,0])  #len(invIndex)=all_num

    jaccard_dist = torch.zeros_like(original_dist)
    for i in range(all_num):
        temp_min = torch.zeros(1,gallery_num)
        indNonZero = torch.nonzero(V[i,:], as_tuple=False)[:,0]
        indImages = []
        indImages = [invIndex[ind] for ind in indNonZero]
        for j in range(len(indNonZero)):
