...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, pretrained=False, cut_at_pooling=False,
        ...
        if not self.cut_at_pooling:
            ...
            self.has_embedding = features_size > 0
            ...
            if self.has_embedding:
                self.feat = nn.Linear(out_planes, self.features_size)
                self.feat_bn = nn.BatchNorm1d(self.features_size)
                init.kaiming_normal_(self.feat.weight, mode='fan_out')
                init.constant_(self.feat.bias, 0)
            else:
                # Change the features_size to CNN output channels
                self.features_size = out_planes
                self.feat_bn = nn.BatchNorm1d(self.features_size)
            self.feat_bn.bias.requires_grad_(False)
            if self.dropout > 0:
                self.drop = nn.Dropout(self.dropout)
            if self.num_classes > 0:
                self.classifier = nn.Linear(self.features_size, self.num_classes, bias=False)
