...
from uda.trainers import ContrastiveMLTrainer
...
def main_worker(args):
    ...
    log_file_name = f'cml_{args.dataset_source}-2-{args.dataset_target}_{args.arch_teacher}_log.txt'
    ...
    for epoch in range(args.epochs):
        ...
        # Trainer
        trainer = ContrastiveMLTrainer(model_1, model_2, model_1_ema, model_2_ema, xbm_teacher_1, xbm_teacher_2,
                                       target_train_loader, optimizer, num_cluster=args.num_clusters, alpha=args.alpha,
                                       num_negative=args.negative_sample_num, contrastive_loss_margin=args.contrastive_loss_margin,
                                       contrastive_loss_temperature_teacher=args.contrastive_loss_temperature_teacher)

        iters = len(target_train_loader)
        if args.peer_and_ema_model_comparison:
            result_1, result_2, losses_peer_ema = trainer.train(epoch, print_freq=args.print_freq, train_iters=iters,
            ...
            with torch.no_grad():
                if not args.use_lab314_dataset:
                    if args.peer_and_ema_model_comparison:
                        ...
                        mAP_1 = mAP_model_1_ema
                        mAP_2 = mAP_model_2_ema
                    else:
