...
from uda.trainers import MoCoTrainer, CAPTrainer
from uda.evaluators import Evaluator, extract_features
from uda.loss import CAPMemory
...
def main_worker(args):
    ...
    log_file_name = f'cl_{args.cl_method}_{args.dataset_source}-2-{args.dataset_target}_{args.arch_teacher}_log.txt'
    ...
    # Create model
    model_1, model_2, model_1_ema, model_2_ema = create_model(args, len(dataset_target.train), device=device)

    if args.cl_method == 'cap':
        cap_memory = CAPMemory(beta=0.7, alpha=0.2, crosscam_epoch=5, bg_knn=10).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        # Trainer
        if args.cl_method == 'moco':
            trainer = MoCoTrainer(model_1, model_2, model_1_ema, model_2_ema, target_train_loader, optimizer,
                                  num_cluster=args.num_clusters, alpha=args.alpha, temperature=0.7)
        elif args.cl_method == 'cap':
            intra_id_features = []
            intra_id_labels = []
            for cc in np.unique(all_img_cams):
                percam_ind = np.where(all_img_cams == cc)[0]
                percam_feature = t_f[percam_ind].numpy()
                percam_label = pseudo_labels[percam_ind]
            ...
            trainer = CAPTrainer(model_1, model_2, model_1_ema, model_2_ema, cap_memory, target_train_loader,
                                 optimizer, num_cluster=args.num_clusters, alpha=args.alpha)
        else:
            raise NameError('Not supported cl method !')

        iters = len(target_train_loader)

        if args.cl_method == 'moco':
            result_1, result_2 = trainer.train(epoch, print_freq=args.print_freq, train_iters=iters)
        elif args.cl_method == 'cap':
            result_1, result_2 = trainer.train(epoch, intra_id_features=intra_id_features, all_img_cams=all_img_cams,
                                               all_pseudo_label=pseudo_labels, print_freq=args.print_freq, train_iters=iters)
        else:
            raise NameError('Not supported cl method !')
        ...
        if ((epoch+1)%args.eval_step==0 or (epoch==args.epochs-1)):
            ...
            with torch.no_grad():
                if not args.use_lab314_dataset:
                    evaluator_ema_1 = Evaluator(model_1_ema)
                    evaluator_ema_2 = Evaluator(model_2_ema)
                    mAP_1 = evaluator_ema_1.evaluate(target_test_loader, dataset_target.query, dataset_target.gallery, cmc_flag=False)
                    mAP_2 = evaluator_ema_2.evaluate(target_test_loader, dataset_target.query, dataset_target.gallery, cmc_flag=False)
                else:
                    mAP_1 = result_1
                    mAP_2 = result_2
    ...
    if not args.use_lab314_dataset:
        ...
        with torch.no_grad():
            ...
            copy_state_dict(checkpoint['state_dict'], model_1_ema)

            evaluator_ema_1 = Evaluator(model_1_ema)
            evaluator_ema_1.evaluate(target_test_loader, dataset_target.query, dataset_target.gallery, cmc_flag=True)
...
if __name__ == '__main__':
    ...
    # CL method
    parser.add_argument('--cl-method', type=str, default='moco', metavar='cl method type')
    # path
