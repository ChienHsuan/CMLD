...
python examples/cl_comparison.py -ds ${SOURCE} -dt ${TARGET} --arch-teacher ${ARCH_TEACHER} \
	--num-instances 4 --lr 0.0001 --iters ${ITERS} -b 16 --epochs 40 \
	--height 256 --width 128 --features ${FEATURES} --cl-method ${CL_METHOD} \
	--memory-bank-size 6400 --negative-sample-num 10 --contrastive-loss-margin 0.3 \
	--contrastive-loss-temperature-teacher 0.7 --rho ${RHO} \
	--init-t1 ${INIT_T1} \
	--init-t2 ${INIT_T2} \
	--data-dir ${DATA_DIR} \
	--logs-dir train_logs/cl_${CL_METHOD}_${SOURCE}-2-${TARGET}_${ARCH_TEACHER}/ \
	--eval-step 1 \
	--print-freq 100
